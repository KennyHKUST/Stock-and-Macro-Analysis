import MySQLdb,time,os,csv #import libraries
...
rootdir="/Users/lixinran/Documents/LiXi/Update Python"
...
for file in filefolder: #iterate files in the folder
    ...
    #print filepath
    with open(filepath,"rb") as csvfile: #loop to open CSV files in the folder
        ...
        for row in spamreader:
            ...
            fy=row[3] #obtain the fiscal year from the forth cell in the row
            fyin=fy[-4:]   #Obtain the last four characters of fy, i.e(fy2016-2016)
            print ric
            ...
            tmpTS = time.strptime(date,"%d/%m/%Y")
            ...
            sqlin="INSERT INTO trweb.tr_report_annual (ric, ts"            
            for i in range(4,34): #iterate the columns in the csv from fy to SGA Expense
                sqlin+=","+colDict[i-3] #add the targeted inserted column into mysql code 
            sqlin+=")VALUES ("+"'"+str(ric)+"','"+str(indate)+"','"+str(fyin)+"'"
            for i in range(4,33): #iterate the columns in the csv from revenue to SGA Expense 
                if row[i]=="": #insert NULL if the data item is empty (data unavailable)
                    ...
                    try:                    
                        data=int(row[i]) #the updated data contains integers in most of columns.
                    ...
                    for row in results:
                        for i in range(4,33):
                            ...
                            if sqlColData[sqlcolDict[i-2]]==None: #convert None into NULL
                                ...
                                sqlColData[sqlcolDict[i-2]]=int(row[i]) #convert the float into integer/
                                #need to be changed when retrieved data is no longer integer later
                            if sqlColData[sqlcolDict[i-2]]!=csvColData[sqlcolDict[i-2]]: #by item, if the data in csv is different from the one in mysql
                                print sqlColData[sqlcolDict[i-2]]
                                print csvColData[sqlcolDict[i-2]]
                                print "bad"
                                sqlupdate="Update trweb.tr_report_annual set " #construct sql code
